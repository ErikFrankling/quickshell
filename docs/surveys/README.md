...
| `metric-centred.qml` / `.md` | The same question with the turned name rejected on looks: twenty-three layouts in which nothing is rotated and nothing sits in the flank, drawn at the rail's true 58px and measured for claim, ink, width, smallest type and rail delta — plus the WCAG contrast of every single `Text` against the ground it actually stands on, washed or plain, worked out by geometry rather than declared, and swept across all nine curated palettes. Why the total cannot go inside a critical ring, why a 7px caption is free *because* it is too small, and why the 9px flank was never wasted in the first place. |
...
| `urgent-mark.qml` / `.md` / `urgent-on.png` / `urgent-cleared.png` | How does a workspace say a window in it wants attention — what `xdg-activation-v1` and `misc:focus_on_activate` actually do, why `hyprctl` cannot see the flag at all, and what the socket-2 event is called, read off the socket rather than guessed. Which six of sixteen shells implement it and what each draws, plus both halves of his own waybar's mark, which turns out to be a 2px edge rule and the label — not a fill. Why the ground is the one channel the pill cannot use for a fifth state, in dL against the shipped ladder and in Lab dE against the pill next to it, on all nine curated palettes. And the trace of it firing on a real link handed to a real browser, and clearing when he goes there. |
...
| `audio-device-select.md` | How do nine of fifteen shells list output and input devices and switch the default — the enumeration filter they all converge on, why every one of them tracks the *whole* candidate list rather than just the two defaults, and which half of `PwNode` is readable without a tracker at all, read off the `.qmltypes` rather than assumed. Also: what this machine actually has (one sink, no sources, four device profiles switched off), why Quickshell cannot turn a profile on, and why testing an input picker with a null sink makes a working filter look broken. |
| `keybind-cheatsheet.md` | Of sixteen shells, which have a keybind cheatsheet (two), which reads `hyprctl binds -j` (one, and only to detect conflicts), and which draws a physical keyboard (none)? What each does when a bind has no description, and why both cheatsheets parse a config file for its comments rather than asking the compositor. Also the finding that sank the original plan: `hyprctl binds -j` emits invalid JSON on Hyprland 0.56.0 — the values are one position out of step with the keys — with the offending bytes quoted. |
